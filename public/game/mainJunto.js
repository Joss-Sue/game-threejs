...
import { cargarEscenario } from './Escenario.js';
...
function actualizarPersonaje(delta) {
    ...
    position: personajePrincipal.position,
    rotation: personajePrincipal.quaternion,
  });
...
socket.on('connect', () => {
  socket.emit('obtener-info-jugador'); // Cliente pide su identidad
});

socket.on('info-jugador', ({ nombre, numero }) => {
  jugadorNombre = nombre;
  jugadorNumero = numero; // 1 o 2
});

// Enviar tu posición y rotación al servidor
function enviarEstado() {
  if (!personajePrincipal) return;

  socket.emit('estadoJugador', {
    posicion: personajePrincipal.position,
    rotacion: personajePrincipal.quaternion,
  });
}

// Escuchar la posición del otro jugador y actualizar su modelo
let personajeRemoto;
let dronRemoto;

socket.on('estado-remoto', ({ position, rotation }) => {
  if (!dronRemoto) return;
  dronRemoto.position.set(position.x, position.y, position.z);
  dronRemoto.quaternion.set(rotation._x, rotation._y, rotation._z, rotation._w);
});

socket.on('estadoOtroJugador', ({ posicion, rotacion }) => {
  if (!personajeRemoto) return;
  
  personajeRemoto.position.set(posicion.x, posicion.y, posicion.z);
  personajeRemoto.quaternion.set(rotacion._x, rotacion._y, rotacion._z, rotacion._w);
});

// Encontrar el modelo del segundo jugador (ya lo cargas)
function asignarPersonajeRemoto(objeto) {
  personajeRemoto = objeto;
}